package eu.shirogfx.annihilation.manager;
...
public class VotingManager {
	...
	public void start() {
        ...
        for (String map : plugin.getMapManager().getRandomMaps()) {
            ...
            plugin.getScoreboardHandler().teams.get(map).setSuffix(
                    ChatColor.RED + " » " + ChatColor.GREEN + "0 Hlasov");
        }
    ...
    public boolean vote(CommandSender voter, String vote) {
            ...
            if (maps.containsKey(val)) {
                ...
                for (String map : maps.values()) {
                    if (vote.equalsIgnoreCase(map)) {
                        ...
                        voter.sendMessage(prefix + " §7Hlasoval si za mapu "
                                + ChatColor.AQUA + map);
                        updateScoreboard();
                        return true;
                    }
        ...
        voter.sendMessage(prefix + " §cMapa §b" + vote + ChatColor.RED + " neexistuje!");
        return false;
    }
